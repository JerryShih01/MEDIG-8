...
export const searchMedicalArticles = async (topic: string, startDate: string, endDate: string): Promise<SearchResult[]> => {
  ...
  const prompt = `
    ...
    CRITICAL: Return the result ONLY as a raw JSON array. Do not use Markdown formatting. Do not include any introductory text like "Here are the results".
    ...
    [
      {
        ...
        "source": "Source name",
        "url": "URL",
        "summary": "Brief summary in Traditional Chinese (Taiwan)",
  ...
  try {
    ...
    // Find the first '[' and last ']' to handle potential conversational text
    ...
    try {
        ...
        // Fallback: try to parse line by line if it's not valid JSON but looks like a list
        throw new Error("無法解析搜尋結果，請稍後再試。");
    }
...
const App: React.FC = () => {
  ...
  const handleSearch = useCallback(async (topic: string, start: string, end: string) => {
    ...
    try {
      ...
      else if (err instanceof Error) rawMsg = err.message;
      else if (err?.error?.message) rawMsg = err.error.message; // Handle raw JSON object
      else rawMsg = JSON.stringify(err);

      let displayMsg = "搜尋發生錯誤，請稍後再試。";
      
      if (rawMsg.includes("429") || rawMsg.includes("RESOURCE_EXHAUSTED") || rawMsg.includes("quota")) {
        displayMsg = "⚠️ API 使用量已達上限 (Quota Exceeded)。請稍後再試，或檢查您的 Google AI Studio 額度與計費設定。";
      } else if (rawMsg.includes("API Key")) {
        displayMsg = "API Key 未設定，請在 Vercel 環境變數中設定 GEMINI_API_KEY";
      } else {
        // If it's a raw JSON string, try to extract a cleaner message
        try {
           const jsonErr = JSON.parse(rawMsg);
           if (jsonErr?.error?.message) displayMsg = `錯誤: ${jsonErr.error.message}`;
        } catch (e) {
           // Not JSON, use raw if short, or default
           if (rawMsg.length < 200) displayMsg = `錯誤: ${rawMsg}`;
        }
      }
